...
function evaluate(str) {
    ...
    let result = [];
    let compileStr = tokenize(compile(str));
    for (let index = 0; index < compileStr.length; index++) {
        let element = String(compileStr[index]);
        if(isNumeric(element) || isDigit(element)){
            result.push(Number(element));
        } else{
            var var1 = result.pop();
            if(element == "+"){
                result.push(result.pop()+var1);
            }else if(element == "-"){
                result.push(result.pop()-var1);
            }else if(element == "/"){
                result.push(result.pop() / var1)
...
function clickHandler(event) {
    ...
    var spanContent = document.getElementsByTagName("span")[0].textContent;
    if (text == "C"){
        document.getElementsByTagName("span")[0].textContent = "";
    } else if (text == "="){
        document.getElementsByTagName("span")[0].textContent = "";
        document.getElementsByTagName("span")[0].textContent += evaluate(spanContent);
    } else{
        document.getElementsByTagName("span")[0].textContent += event.target.textContent;
    }
}
...
window.onload = function (event) {
    ...
    elem2.addEventListener("click", clickHandler)
}
